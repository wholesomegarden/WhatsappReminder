...
// https://github.com/open-wa/wa-automate-python/blob/master/src/js/wapi.js
...
window.WAPI.getMessageQuote = async function (quotedStanzaID, res) {
	const out = await Store.Msg.models.filter((chat) => chat.quotedStanzaID != undefined).map(
		(quotedStanzaID) => ({
			quotedID: quotedStanzaID,
		})
	)
	return out
};
//
// window.WAPI.getMessageQuote = async function (msg) {
// 	return Store.Msg.models.filter((chat) => chat.quotedStanzaID != undefined).map(
// 		(msg) => ({
// 			quotedID: msg.quotedStanzaID,
// 			quotedParticipant: msg.quotedParticipant._serialized,
// 			quotedMessage: msg.quotedMsg.body,
// 			quotedType: msg.quotedMsg.type,
// 		})
// 	)
// 	return true
// };
...
window.WAPI.simulateTyping = async function (chatId, on) {
  if (on != '0' ) Store.ChatStates.sendChatStateComposing(chatId)
 ...
 * Ghost forwarding is like a normal forward but as if it were sent from the host phone.
 */
window.WAPI.ghostForward2 = async function (chatId, messageId) {
	var chat = Store.Chat.get(chatId);
	if (!Store.Msg.get(messageId)) return false;
	var tempMsg = Object.create(Store.Msg.get(messageId));
	var newId = window.WAPI.getNewMessageId(chatId);
	var toId = new Store.WidFactory.createWid(chatId)
	var extend = {
		...JSON.parse(JSON.stringify(tempMsg)),
		ack: 0,
		id: newId,
